...
public function checkversion(){
	if($this->phpversion===0){
		if (strnatcmp(phpversion(),'5.4.0') >= 0){
			return $this->phpversion = 1;
		}
		else{
			return $this->phpversion = 2;
		}
	}
	else{
...
//end version
	...
	public function generateSignatureAndEncrypt($parameters = array()) {
		...
		try{
			...
			$this->executeAfterApi ( $startTime, 'SIGN_AND_ENCRYPT' );
		}catch(Exception $e) {
	...
	public function getProcessPaymentUrlWithQueryParameters($queryParameters, $redirectUrl){
       ...
       * @optional requestParameters['created_time_range'] - [Array]
       */
	...
	private function encrypt($K, $IV, $P = null, $A = null, $tag_length = 128) {
		...
		try {
			...
			assert ( in_array ( $tag_length, array(
			128,
			120,
			112,
			104,
	...
	private function getPublicKey() {
		if( !file_exists ( dirname(__DIR__)."/Amzpay/config.ini" ) ){
			throw new Exception( "Config file does not exist", 1 );
		}
		$config = parse_ini_file ( dirname(__DIR__)."/Amzpay/config.ini" );
	...
	private function common($K, $key_length, $IV, $A) {
		...
		if($this->checkversion()===1){
			$H = openssl_encrypt ( str_repeat ( "\0", 16 ), 'aes-' . ($key_length) . '-ecb', $K, OPENSSL_NO_PADDING | OPENSSL_RAW_DATA );
		 	...
		 	$H = mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $K ,str_repeat ( "\0", 16 ), MCRYPT_MODE_ECB);
		}
	...
	private function getLSB($num_bits, $x) {
		...
		if($this->checkversion()===1){
			...
			$tot = strlen($x);
			return substr ( $x, $tot - $num_bytes, $tot );
		}
	}
	...
	private function getHash($H, $X) {
		...
		if($this->checkversion()===1){
			...
			for($i = 1; $i <= $num_blocks; $i ++) {
				$Y [$i] = $this->getProduct ( $this->getBitXor ( $Y [$i - 1], substr ( $X, ($i - 1) * 16, 16 ) ), $H );
			}
		}
	...
	private function getGCTR($K, $key_length, $ICB, $X) {
		...
		if($this->checkversion()===1){
			...
			for($i = 1; $i < $n; $i ++) {
				$C = mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $K ,$CB [$i], MCRYPT_MODE_ECB);
				
				$Y [$i] = $this->getBitXor ( substr ( $X, ($i - 1) * 16, 16), $C );
			}
			$Xn = substr ( $X, ($n - 1) * 16, 16 );
			$C = mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $K ,$CB [$n], MCRYPT_MODE_ECB);
		...
		try {
			...
			do {
				try {
					...
					$response = $httpCurlRequest->httpGet($this->mwsServiceUrl.'?'.$parameters, $this->userAgent);
	...
	/* Collapse multiple whitespace characters into a single ' ' and backslash escape '\',
			...
			*/
			...
			private function updateLatencyMetrics($totalTime, $operation){
				$file = dirname(__DIR__).'/Amzpay/metrics/latencyMetrics.txt';
			...
			private function updateCountMetrics($metrics){
				$file = dirname(__DIR__).'/Amzpay/metrics/countMetrics.txt';
				$sellerId = $this->config ['merchant_id'];
			...
			private function checkWritePermissions(){
				if(!(is_writable(dirname(__DIR__).'/Amzpay/metrics'))){
					throw new Exception("Metrics Directory is not writable", 1);
				}
				if(!(is_writable(dirname(__DIR__).'/Amzpay/config.ini'))){
