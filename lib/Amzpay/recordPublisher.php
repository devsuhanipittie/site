...
function postMetrics(){
	...
	if(($config['lastFetchedTimeForPostingMetrics']<(time()-300))||($metricsSize)>500000){
		...
		$fp = fopen(dirname(__DIR__).'/PayWithAmazon/config.ini', 'w');
		flock($fp, LOCK_EX);
		...
		flock($fp, LOCK_UN);
		fclose($fp);
		if(file_exists(dirname(__DIR__).'/PayWithAmazon/metrics/latencyMetrics.txt')){
			unlink(dirname(__DIR__).'/PayWithAmazon/metrics/latencyMetrics.txt');
		}
		if(file_exists(dirname(__DIR__).'/PayWithAmazon/metrics/countMetrics.txt')){
			unlink(dirname(__DIR__).'/PayWithAmazon/metrics/countMetrics.txt');
		}
	}
}
...
function parseLatencyFile(){
	...
	if(file_exists(dirname(__DIR__).'/PayWithAmazon/metrics/latencyMetrics.txt')){
		$lines = file(dirname(__DIR__).'/PayWithAmazon/metrics/latencyMetrics.txt');
		$title = 'data';
		$json_data=array();
...
function parseCountFile(){
	...
	if(file_exists(dirname(__DIR__).'/PayWithAmazon/metrics/countMetrics.txt')){
		$lines = file(dirname(__DIR__).'/PayWithAmazon/metrics/countMetrics.txt');
		$title = 'data';
		$json_data=array();
		foreach ($lines as $line_num => $line) {
			preg_match($pattern,$line,$result);
