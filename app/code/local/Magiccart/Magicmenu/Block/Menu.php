    ...
    public function drawMainMenu()
    {
        if(!Mage::registry('drawMainMenu')){
            ...
            foreach ($catListTop as $catTop) :
                ...
                $url =  '<a class="level-top" href="' . Mage::getUrl($catTop->getUrlPath()). '"><span>'
                ...
                if($catTop->hasChildren()) {
                    $mobileUrl =  '<a class="level-top" href="#"><span>'
                        .$this->__($catTop->getName()).' '.$this->getCatLabel($catTop). //.'<img src="'.$this->getImage($catTop).'" alt="'.$this->__($catTop->getName()).'" />
                        '</span><span class="boder-menu"></span></a>';
                } else {
                    $mobileUrl =  '<a class="level-top" href="' . Mage::getUrl($catTop->getUrlPath()). '"><span>'.$this->__($catTop->getName()).' '.$this->getCatLabel($catTop).
                        '</span><span class="boder-menu"></span></a>';
                }
                $mobileUrlViewAll =  '<a href="' . Mage::getUrl($catTop->getUrlPath()). '">View ALL '.$this->__($catTop->getName()).' '.$this->getCatLabel($catTop).
                    '</a>';
                ...
                $mobileMenu .= '<li class="level-top">'.$mobileUrl;
                            ...
                            $drawMainMenu .='<div class="content-mega">';  /*  Content Mega */
                                ...
                                $drawMainMenu .= '<div class="content-mega-horizontal">';
                                    ...
                                    if($nChild) :
                                        ...
                                        $mobileMenu .= '<ul>';
//                                        $mobileMenu .= '<li>'.$mobileUrlViewAll.'</li>';
                                        $first ="level-3 first";
                                        foreach ($childTop as $child) {
                                            ...
                                            if($child->hasChildren()) {
                                                $mobileUrl = $this->getImage($child) . '<a href="#">' . $this->__($child->getName()) . ' ' . $this->getCatLabel($child) . '</a>';
                                            } else {
                                                $mobileUrl =  $this->getImage($child) .'<a href="'. Mage::getUrl($child->getUrlPath()).'">'.$this->__($child->getName()). ' '.$this->getCatLabel($child) . '</a>';
                                            }
                                            $mobileUrlViewAll =  $this->getImage($child) .'<a href="'. Mage::getUrl($child->getUrlPath()).'">View All '.$this->__($child->getName()). ' '.$this->getCatLabel($child) . '</a>';
                                            $drawMainMenu .= '<li class="children"><ul>';
                                                $drawMainMenu .= '<li class="'.$first.'">'.$url.'</li>';
                                            $mobileMenu .= '<li>'.$mobileUrl;
                                                $catList = $this->getCatByPath($child->getEntityId(), $child->getPath());
                                                if(count($catList)){
                                                    $mobileMenu .='<ul>';
//                                                    $mobileMenu .= '<li class="nav '.$first.'">'.$mobileUrlViewAll.'</li>';
                                                    $first = ''; // $i = 0; $last  = '';
