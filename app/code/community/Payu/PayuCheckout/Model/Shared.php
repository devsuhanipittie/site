    ...
    public function getResponseOperation($response)
    {
        ...
        if (isset($response['status'])) {
            ...
            if ($response['status'] == 'success') {
                ...
                if ($order->getId()) {
                    $grandTotal = number_format($order->getBaseGrandTotal(), 0, '', '');
                    //$response['amount'] = 50;
                    if ($grandTotal != $response['amount']) {
                        $incId = $order->getIncrementId();
                        $responseAmt = $response['amount'];
                        Mage::log('order canceled due to amount mismatch ---------------------- ',null,'wagentopayu.log');
                        Mage::log("Order Id : $incId",null,'wagentopayu.log');
                        Mage::log("Order Response Amount : $responseAmt",null,'wagentopayu.log');
                        Mage::log("Order Grand Total : $grandTotal",null,'wagentopayu.log');
                        $postData = array('id' => $incId);
                        $failureUrl = Mage::getUrl('payucheckout/shared/failure',$postData);
                        $response = Mage::app()->getFrontController()->getResponse();
                        $response->setRedirect($failureUrl);
                    ...
                    Mage::log("$orderid not exist in database", null,'wagentopayu.log');
                }
            }
